...
# Inicializar servicios con lazy loading
reconocedor = None
gestionador = None
detector = None
...
def capturar_rostro():
    try:
        ...
        get_reconocedor().capturar_rostro(codigo_estudiante)
        limpiar_memoria()
...
def verificar_rostro():
    try:
        ...
        codigo, confianza = get_reconocedor().reconocimiento_facial(imagen)
...
def registrar_pertenencia():
    try:
        ...
        ruta_imagen = get_detector().guardar_imagen(imagen, codigo_estudiante, tipo_objeto)
        if not ruta_imagen:
            return jsonify({'error': 'Error al guardar la imagen'}), 500

        resultado = get_gestionador().registrar_pertenencia(
...
def consultar_pertenencias():
    try:
        ...
        resultado = get_gestionador().obtener_pertenencias(codigo_estudiante)
        return jsonify(resultado)
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def detectar_objetos():
    try:
        ...
        # Detectar objetos
        objetos = get_detector().detectar_objetos(imagen)
...
def registrar_estudiante():
    try:
        ...
        for i, imagen_base64 in enumerate(imagenes_base64):
            ...
            rostros = get_reconocedor().detector.detectMultiScale(
                gris,
                scaleFactor=1.1,
                minNeighbors=5,
                minSize=(30, 30)
        ...
        get_reconocedor().entrenar_modelo()
        
        db = get_gestionador().db
...
def reporte_pertenencias_pdf():
    try:
        registros = get_gestionador().obtener_pertenencias()
        buffer = io.BytesIO()
...
def registrar_salida_pertenencia():
    try:
        ...
        if not codigo_estudiante or not tipo_objeto:
            return jsonify({'error': 'Faltan datos requeridos'}), 400
        resultado = get_gestionador().registrar_salida(codigo_estudiante, tipo_objeto)
        return jsonify(resultado)
    except Exception as e:
        return jsonify({'error': str(e)}), 500
